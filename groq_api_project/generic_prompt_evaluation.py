...
def generate_prompt_evaluation_report(evaluation_results):
  ...
  pass_rate = (
      100 * len([s for s in scores if s >= 7]) /
...
dataset = evaluator.generate_dataset(
    ...
    prompt_inputs_spec={
        "height": "Athelete's height in centimeters",
        "weight": "Athelete's weight in kilograms",
        "goal": "Athelete's goal (e.g., weight loss, muscle gain, maintenance)",
        "restrictions": "Any dietary restrictions or preferences (e.g., vegetarian, gluten-free, allergies)",
    },
...
results = evaluator.run_evaluation(
    ...
    - Meals with exact foods, portion sizes, and timing
    """
)
